...
| 关卡 | 状态 |
|---|---|
| 第一关 · 数据看板 | **已完成**：两个指标接口按 KB-001 口径实现并通过对拍验证，看板做好，本文件写好 |
| 第二关 · 修好 RAG 服务 | 进行中，`DEBUG_LOG.md` 待写 |
| 第三关 · 混合问答 | 未开始，`LLM_SETUP.md` 待写 |
| 第四关 · 可调试性 | 未开始 |

下面「接口」一节里 `/api/retrieve`、`/api/chat`、`/api/trace/{trace_id}` 三个接口是 starter 原有的、目前尚未按第三关要求修好，
接口形状与契约一致，但回答质量还没有达标。第一关交付的接口是 `/api/health`、`/api/metrics/*`、`/api/meta`、`/api/data_quality`。
...
```bash
cd starter && .venv/bin/python -m pytest tests -q     # 76 passed
```
...
1. **BM25 检索依赖字面词。** 同义改写（文档里写「退款」，用户问「退钱」）只能靠知识库里的别名词典兜底，词典没覆盖到的说法会漏。加向量检索能缓解，代价是引入模型下载或外部 Key。目前优先保证「干净环境能跑起来」。
